...
W/S 油门/制动倒车，A/D 转向，空格手刹，C / F1 切换八种视角，默认自动换挡，Q 临时降挡，**R 回到最近路面并使当前圈失效**，Esc 暂停。手柄 RT/LT 油门/制动，左摇杆转向，Start 暂停。
...
默认自动挡，起步一挡。Q 或手柄左肩键临时降挡，保持自动换挡，带超转保护；短暂保挡后继续自动升挡。E/M 不再切换挡位或传动模式。S 制动，停车后继续按 S 倒车。
选赛道仅更新轻量路线预览，点击开始后才加载实际场景；加载时锁定设置并显示加载状态。近追车、远追车、驾驶舱、仪表台、引擎盖、保险杠、头盔和纯路面八个机位固定视野，C/F1 切换。
